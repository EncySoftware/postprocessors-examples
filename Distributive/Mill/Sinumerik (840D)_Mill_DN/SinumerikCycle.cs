...
public class SinumerikCycle
{
    ...
    public void OutCycle(string ACycleID, string CycleGeomName)
    {
        ...
        if (State.Cyclecompare != sss)
        {       //Dobavil variant dlya togo, chtoby vyvodit massiv otverstij odnim cziklom
            if (!IsFirstCycle && !Cycle_pocket)
            {          //menyaetsya parametr, nuzhno zakryt czikl
