...
public partial class Postprocessor : TPostprocessor
{
    ...
    public override void OnStartTechOperation(ICLDTechOperation op, ICLDPPFunCommand cmd, CLDArray cld)
    {
        ...
        {
            INamedProperty ToolProps = cmd.Ptr["PPFun(TechInfo).Operation(0).ToolSection.Tools(0)"];
            double ToolF1 = ToolProps.Flt["F1"];
            double ToolWidth = ToolProps.Flt["InsertF"];
            if (ToolF1 > 6)
            {
                barFeedZero = barFeedZero + (ToolF1 - ToolWidth);
            }
        }
        ...
        if (currentOperationType == OpType.Lathe)
        {
            ...
            xScale = 2.0; //1.0; It seems milling is also in diameters
    }
    ...
    public override void OnSpindle(ICLDSpindleCommand cmd, CLDArray cld)
    {
        ...
        if (cmd.IsOn)
        {
            ...
            if ((cmd.IsClockwiseDir && nc.MSpindle == 3) || (!cmd.IsClockwiseDir && nc.MSpindle == 4))
            {
                nc.MSpindle.Show(5);
                nc.Block.Out();
            }
            ...
            nc.MSpindle.Show(spindleGroup + Mcod);
            nc.Block.Out();
        }
        else if (cmd.IsOff)
        {
    ...
    public override void OnLoadTool(ICLDLoadToolCommand cmd, CLDArray cld)
    {
        ...
        if (op.Tool != null && op.Tool.Command != null)
        {
            ...
            if ((currentOperationType == OpType.Lathe) || (currentOperationType == OpType.Auxiliary))
            {
                nc.Y.v = 0;
                nc.Y.v0 = 0;
            }
    ...
    private bool IsSpindleStopped()
    {
        return ((nc.MSpindle.v == 5) || (nc.MSpindle.v >= 10));
    }
    ...
    public override void OnPhysicGoto(ICLDPhysicGotoCommand cmd, CLDArray cld)
    {
        foreach (CLDMultiMotionAxis ax in cmd.Axes)
        {
            if (SameText(ax.ID, "AxisX1Pos") || SameText(ax.ID, "AxisX2Pos"))
            {
                nc.U.Show(0);
            }
            else if (SameText(ax.ID, "AxisY1Pos") || SameText(ax.ID, "AxisY2Pos"))
            {
                nc.V.Show(0);
            }
            else if (SameText(ax.ID, "AxisZ1Pos") || SameText(ax.ID, "AxisZ2Pos"))
            {
                nc.W.Show(0);
            }
        }
        if (nc.U.Changed || nc.W.Changed || nc.W.Changed)
        {
            nc.C.v0 = 0;
            nc.C.v = 0;
            nc.C.Hide(0);
    ...
    public override void OnGoHome(ICLDGoHomeCommand cmd, CLDArray cld)
    {
        if (IsLastGoHome(cmd))
        {
            ...
            if (SameText(CurrentOperation.TypeName, "TSTBarFeeding"))
            {
                SynchPoints.Add();
                //nc1.WriteLine("M01");
                nc2.WriteLine("M01");
    ...
    {
        if (IsPNPoperation(CurrentOperation))
            return;
    ...
    public override void OnHoleExtCycle(ICLDExtCycleCommand cmd, CLDArray cld)
    {
        ...
        else if (cmd.IsCall)
        {
            ...
            if (planeZIndex == 1)
            {
                nc.PlaneZReg.v = nc.PlaneZReg.v * xScale;
                nc.RSafeLevel.v = nc.RSafeLevel.v * xScale;
            }
            if (cld[9] == 0)
                nc.F.v = cld[10] * nc.S;
            else
            ...
            if (currentOperationType == OpType.Lathe)
            {
                switch (cmd.CycleType)
                {
                    ...
                    case CLDConst.W5DTap:
                        if (cld[19] == 1)
                            nc.WriteLine("M29 " + nc.S.ToString(nc.S.v));
                        nc.GFeed.Show(99);
                        nc.F.v = cld[10];
                        nc.GCycle.v = 84;
                        nc.RSafeLevel.Hide(); //nc.RSafeLevel.Show(cld[6]);
                        nc.PlaneZReg.Show();
                        nc.F.Show();
                        if (cld[15] != 0)
    ...
    public void outPause(double Sec, int ChannelFile = 0)
    {
        ...
        n.Output("G04 P" + (Sec * 1000).ToString());
    }
    public void outAirBlower(double Sec)
    {
        nc.Output("M28 (Air)");
        outPause(Sec);
    }

    public void OutDetectBrokenTool()
    {
        nc.WriteLine("M36");
        outPause(0.5, activeChannel + 1);
    }
    ...
    public void OutBarFeeding(float Overhang, TInp3DPoint TouchPos, int ToolNum, int ToolCor, float SafeDist)
    {
        ...
        nc.WriteLine(nc.GInterp.ToString(150) + nc.Z.ToString(barFeedZero));
        outClamp(true, true);
        nc.GInterp.Show(0);
        nc.GFeed.Show(99);
        nc.Z.Show(-SafeDist);
        nc.Block.Out();
        //nc.WriteLine(nc.GInterp.ToString(0) + nc.GFeed.ToString(99) + nc.Z.ToString(-SafeDist));
        nc.WriteLine("G00 S1000 M13");
    ...
    public override void OnPickAndPlaceExtCycle(ICLDExtCycleCommand cmd, CLDArray cld)
    {
        switch (cmd.CycleType)
        {
            case CLDConst.WBarFeedingCycle:
                ...
                OutBarFeeding((float)cld[3], TouchPos, (int)cld[4], (int)cld[5] + 30, ((float)cld[9]));
                break;
