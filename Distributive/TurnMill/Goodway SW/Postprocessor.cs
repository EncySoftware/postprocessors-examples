...
public partial class Postprocessor : TPostprocessor
{
    ...
    ///<summary>Dlya opredeleniya kol-va tokarnyx cziklov v odnoj operaczii</summary>
    int CycleOpnum = 0;
    ...
    void InitNCFile(out NCFile nc, int ProgNum, bool isSecondChannel)
    {
        ...
        if (isSecondChannel)
            suffix = "_2";
        nc.WriteLine("%");
        nc.WriteLine("O" + nc.ProgName + " (" + Transliterate(CLDProject.ProjectName) + suffix + ")");
    ...
    void outPartEjection()
    {
        ...
        outClamp(nc, false, false, 1);
        nc.M.Show(15); //air off
        nc.Block.Out();
    ...
    void OutComment(string cmt)
    {
        var s = "N" + opNCounter + "( " + Transliterate(cmt) + " )";
        nc1.OutWithN(s);
        nc2.OutWithN(s);
    ...
    public void OutBrakeC(bool IsOn)
    {
        if (IsOn)
            nc.MCBrake.v = 6;
        else
    ...
    public override void OnAxesBrake(ICLDAxesBrakeCommand cmd, CLDArray cld)
    {
        ...
        {
            if ((ax.IsC1 && (nc == nc1)) || (ax.IsC2 && (nc == nc2)))
            {
                OutBrakeC(ax.StateIsOn);
            }
        }
    }
    ...
    public void outPause(double Sec, NCFile nc)
    {
        nc.Output("G4 " + nc.XDelay.ToString(Sec));
    }
    public void outClamp(NCFile nc, bool MainSpindle, bool ClampOn, double delay)
    {
        int Mcode = 10;
        int ClampIndex = 0;
        ...
        if (ClampState[ClampIndex] != Mcode)
        {
            ...
            if (delay > 0)
                outPause(delay, nc);
            if (ClampOn)
                nc.Output("M15 (Air off)");
        }
    }
    public void OutBarFeedBody(NCFile nc, float Overhang)
    {
        nc.MCoolant.v = 26;
        nc.Block.Out();
        outPause(0.2, nc);
        ...
        nc.WriteLine("G62");
        outPause(0.2, nc);
        outClamp(nc, true, false, 0.2);
        nc.WriteLine("G75");
        nc.WriteLine("M00");
        outClamp(nc, true, true, 0.2);
        nc.WriteLine("G63");
        outPause(0.2, nc);
    }
    public void MakeBarFeedSub(int ProgNum, float Overhang)
    {
        NCFile sub = null;
        InitNCFile(out sub, ProgNum, false);
        OutBarFeedBody(sub, Overhang);
        sub.WriteLine("M99");
        sub.WriteLine("%");
    }
    public void OutBarFeeding(float Overhang, TInp3DPoint TouchPos, int ToolNum, int ToolCor, float SafeDist)
    {
        ...
        try
        {
            nc.WriteLine(nc.GInterp.ToString(0) + nc.W.ToString(-SafeDist));
            nc.WriteLine(nc.GInterp.ToString(30) + nc.U.ToString(0));
            // double ToolWidth = 2;//CurrentOperation.Tool.;
            // nc.WriteLine(nc.GInterp.ToString(0) + nc.W.ToString(ToolWidth));
            if (Settings.Params.Bol["OutFiles.BarFeedAsSub"])
            {
                int SubNum = Settings.Params.Int["OutFiles.BarFeedSubNumber"];
                nc.WriteLine(nc.M.ToString(98) + nc.P.ToString(SubNum));
                MakeBarFeedSub(SubNum, Overhang);
            }
            else
            {
                OutBarFeedBody(nc, Overhang);
            }
        }
        finally
        {
    ...
    public void SyncZ(bool IsOn, bool OnlyActiveNC)
    {
        int MCode = 41; //Sync Z/ZB off
        if (IsOn)
            MCode = 40; //Sync Z/ZB on
        if (OnlyActiveNC)
        {
            nc.MSyncZ.v = MCode;
            nc.Block.Out();
    ...
    public void SyncSpindles(bool IsOn, bool OnlyActiveNC)
    {
        int code = 83;  //M83 (desync spindles)
        if (IsOn) code = 82;  //M82 (Sync spindles)
        if (OnlyActiveNC)
        {
            nc.MSyncSp.v = code;
            nc.Block.Out();
    ...
    public override void OnInterpCylindrical(ICLDInterpolationCommand cmd, CLDArray cld)
    {
        if (cmd.IsOn)
        {
            ...
            nc.GPolarOrCyl.Show(107);
            nc.C.Show(cmd.CylRadius);
            nc.Y.Address = "C";
            nc.Y.Reset();
            nc.Block.Out();
        }
    ...
    void OutFirstPointBeforePolar(ICLDInterpolationCommand cmd)
    {
        ...
        if (mvm == null)
        {
            Log.Error("Otsutstvuyut peremeshheniya pered vklyucheniem polyarnoj interpolyaczii.");
            return;
        }
